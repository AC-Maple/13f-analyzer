...
**Verification status of the two new export/import fields, stated plainly:** the GICS mnemonics themselves are live-confirmed (see the section above). What is *not* yet confirmed is that they resolve correctly when pulled through the real per-CUSIP `/cusip/{cusip} Equity` template pattern at scale (as opposed to the standalone test workbook's ticker-based pull) — that's a real refresh still owed, tracked the same way `export_bloomberg_template.py`'s own trailing print statement already flags it.
